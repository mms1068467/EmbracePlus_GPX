...
from preprocessing.lookup_tables import freq_dic, cluster_size_dic, interval_dic, butter_filter_order_dic, lowpass_cutoff_freq_dic, highpass_cutoff_freq_dic
...
def filter_signals(data: pd.DataFrame, phys_signal: str) -> pd.DataFrame:
    """
    ...
    z = signal.lfilter(b, a, data[phys_signal])
    filteredGSR = signal.lfilter(c, d, z)

    data[phys_signal] = filteredGSR
            ...
            # adapt time of given values in standard interval (i.e. GSR: 250ms) and interpolate or fill in missing values
                ...
                if number_of_new_values > 0:
                    ...
                    for j in range(j, for_end):
                        ...
                        first_part_data = pd.concat([first_part_data, pd.DataFrame(data=d)], ignore_index=True)
...
def preprocess_for_MOS(file_path: str) -> pd.DataFrame:
    """
...
def geolocate(signal_data: pd.DataFrame, location_data: pd.DataFrame) -> pd.DataFrame:
    ...
    merged_geolocated_data = merged_geolocated_data[['time_iso_x', 'GSR', 'ST', 'IBI', 'HRV', 'latitude', 'longitude', 'speed']]
